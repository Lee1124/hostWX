			...
			<view class="uni-calendar__wrapper">
				<view class="uni-calenda__content">
					<view class="uni-calendar__panel">
						<view class="uni-calendar__date-befor" :class="{'noBefore':noBefore}" @tap="dataBefor(-1, 'month')"><text class="my-iconfont">&#xe84f;</text></view>
						<view class="uni-calendar__panel-box">
							<view>{{ canlender.year }}年</view>
							<view>{{ canlender.month }}月</view>
						</view>
						<view class="uni-calendar__date-after uni-calendar__rollback" @tap="dataBefor(1, 'month')"><text class="my-iconfont">&#xe84f;</text></view>
						<view class="uni-calendar__backtoday" @tap="backtoday" v-if="isShowToDayBtn">回到今天</view>
					</view>
					...
					<uni-calendar-item :canlender="canlender" :lunar="lunar" @selectDays="selectDays" />
				</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			getMonthAll(index, date) {
				...
				// if (parseInt((this.$manjs.getFormatTime(date, '-')).split('-')[1]) == parseInt((this.$manjs.getFormatTime(new Date,
				// 		'-')).split('-')[1])) {
				// 	console.log('进来啦啦啦')
				// 	this.noBefore=true;
				// 	this.isNoChangePrevMonth=true;
				// }else{
				// 	this.isNoChangePrevMonth=false;
				// }
			},
			/**
			 * 设置事件
			...
			dataBefor(id, types) {
				...
				if (id == -1) {
					let day = (this.$manjs.getFormatTime(new Date, '-')).split('-');
					let addMonth;
					if ((parseInt(day[1]) + 1) > 12) {
						addMonth = (parseInt(day[0]) + 1) + '-' + 1 + '-' + day[2];
					} else {
						addMonth = day[0] + '-' + (parseInt(day[1]) + 1) + '-' + day[2];
					}
					if (addMonth == year) {
						this.noBefore = true;
					}
					let yearMonth1=this.canlender.year + '-' + this.canlender.month;
					let yearMonth2=(this.$manjs.getFormatTime(new Date, '-')).split('-')[0]+'-'+(this.$manjs.getFormatTime(new Date, '-')).split('-')[1]
			...
			selectDays(params) {
				...
				if (parseInt((this.$manjs.getFormatTime(time, '-')).split('-')[1]) == parseInt((this.$manjs.getFormatTime(new Date,
						'-')).split('-')[1])) {
					this.noBefore=true;
					this.isNoChangePrevMonth=true;
				}
